...
自分のブラウザでそのURLを開くと、テストが使っているのと同じ卓が観戦者として見える。
Enterを押すとテストが先へ進む。ログとスクリーンショットでは分からない挙動を、
人間が目で見て確かめたいときのためのもの。

**1テストを単独で実行するときに使うもの。** テストごとに1回止まるので、全件実行に
付けると89回止まる。

`--watch` を付けると3つが自動で揃う。どれか1つでも欠けると成立しない。

| | 何のため |
|---|---|
| `docker run -p 10011:10011` | 自分のブラウザからテストサーバへ届くようにする |
| `docker run -i` | コンテナの標準入力を繋ぐ。無いと `input()` が即EOFになる |
| `pytest -s` | 出力キャプチャを止める。無いとURLの表示も入力待ちも実行中には効かない |

ホスト側のポートを10011からずらしてはいけない。テストが持っているURLは
`http://localhost:10011/tables/<名前>` で、それをそのまま人間に渡しているため。
...
- **見えるのは「共有された卓」であって「テストブラウザの画面」ではない。**
  スクリーンショットのリアルタイム版ではない。手札エリアの中身は所有者からしか
  表向きに見えないし、テーブルのパン位置もウィンドウサイズもクライアントごとに独立
- **見るだけにすること。** 観戦者は本来操作できないはずだが、メニューからのキット
  出し入れやカウンターの操作はガードを通り抜ける（#103）。うっかり触ると卓が壊れる
- **観戦した実行の結果を判定に使わない。** 人間のブラウザがホスト側でCPUを食うので、
  タイミングに影響しうる。観戦は「見るため」の道具で、「測るため」の道具ではない
- 止まるのは**誰かが卓に参加し終えた後**でなければならない。プレイヤーが1人も
  いない卓を開いた人は自動的にホストになる（`play_session.js` の `initializeTable`）
  ので、それより前に人間が開くと人間のブラウザがホストになってテストが壊れる。
  そのためポーズは `GameHelper.go()` の直後に置いたうえで、**`should_be_joined()` が
  返るまで待ってからURLを出す**。参加しないまま卓を開くクライアント（観戦者のテスト）
  が先に来たときは、URLを出さずに素通りする
- 止める場所が `GameHelper.go()` なのは、**全テストが必ず通る唯一の場所**だから。
  `host` fixture は半分ちょっとのテストしか使っておらず、残りは `browser` を直に
  受け取って `GameHelper` を自分で作っている
- 操作をゆっくりにしたいときは `ASOBANN_E2E_SLOWMO=1.0` を併用する。効くのは
  クリック・ダブルクリック・ドラッグ・メニュー操作だけで、待ちの区間には効かない
...
詳細は asobann_docs の `worklogs/20260820.e2e-observability-study.WIP/05-history-stats.md`（private）。
優先度の判断はやり直しが必要。
