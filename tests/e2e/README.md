...
**1テストを単独で実行するときに使うもの。** 止まるのは `host` fixture が返った直後で、
これはテストごとに1回ずつある。全件実行に付けると89回止まる。
...
- 止まるのは**ホストが卓に参加し終えた後**でなければならない。プレイヤーが1人も
  いない卓を開いた人は自動的にホストになる（`play_session.js` の `initializeTable`）
  ので、それより前に人間が開くとテストが壊れる。`GameHelper.player()` が
  `should_be_joined()` まで待つので、その後なら人間は必ず観戦者になる
